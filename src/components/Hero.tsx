...
const Hero = () => {
  ...
  const isSustainableRoute = location.pathname === '/sustainable';
  const videoSrc = isSustainableRoute
    ? `${import.meta.env.BASE_URL}sustainable.mp4`
    : homeVideo;
  useEffect(() => {
    const v = videoRef.current;
    if (!v) return;
    v.muted = isSustainableRoute;
    v.load();
